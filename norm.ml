...
let norm ?(weak=false) =
  let rec norm (sigma, gamma as ctx) ((e', loc) as e) =
    ...
    | Const x, loc when is_elim sigma x && not empty_sp -> (* may trigger an iota reduction *)
       iota ctx x sp_e
    | Const x, loc -> 
  ...
  and iota (sigma, gamma as _ctx) const sp =
    let sp_len = List.length sp in
    let t, n, d = Util.this (lookup_elim const sigma) in
    if n == sp_len then
      let x = List.nth sp 0 in
      
      (Print.warning "Unimplemented iota reduction" ;
       join_head_spine (mk_const const) sp)
    else
      (Print.debug "ι reduction is stuck (more parameters required)" ;
       join_head_spine (mk_const const) sp) (* TODO some location information is lost here *)
  in
  norm
      ...
      | App (e1, e2) ->
	   ...
	   (* Beta - reduction *)
	    ...
	    | App _ when head_is_elim sigma e1 -> 
	       ...
	       let delim, _, d = Util.this (lookup_elim x sigma) in
